...
Cache::Cache(int size, int block_size, int associativity, 
	int write_through, int write_allocate, Storage *lower, int prefetch_, int bypass_){
	config_.size=size;
	...
	//printf("Cache Initialize with size %d, associatovity %d, set_num %d, block_size %d\n", size, associativity, config_.set_num, block_size);

	lower_=lower;
	lines=new Line*[config_.set_num];
	last_use=new int*[config_.set_num];
	hit_times=new int*[config_.set_num];
	come_time=new int*[config_.set_num];
	bypass_info=new BypassInfo[config_.set_num];
	for(int i=0;i<config_.set_num;++i){
		lines[i]=new Line[config_.associativity];
		last_use[i]=new int[config_.associativity];
		hit_times[i]=new int[config_.associativity];
		come_time[i]=new int[config_.associativity];
		bypass_info[i].miss=0;
		bypass_info[i].total=0;
	...
	if(debug) printf("Cache construct succeed\n");
}
...
void Cache::HandleRequest(uint64_t addr, int bytes, int read, char *content) {
	...
	if(debug) printf("Start cache request with addr: %lx, bytes: %d, offset: %lx, index: %lx, tag: %lx\n", addr, bytes, offset, index, tag);
	...
	// Bypass?
	if (!(bypass&&BypassDecision(index))) {
		int pos=ReplaceDecision(index,tag);
		if(debug) printf("Hit position: %d\n",pos);
		if(read==1){
			if(pos>=0){
		...
		stats_.access_time+=(latency_.hit_latency+latency_.bus_latency)*clock_cycle;
		bypass_info[index].total++;
  	}
	else{
	...
	if(debug) printf("End cache request\n");
}

int Cache::BypassDecision(uint64_t index) {
	if(debug) printf("Begin bypass decision\n");
	int total=bypass_info[index].total;
	int miss=bypass_info[index].miss;
	if(total>=6 && ((double)miss)/total>0.3)
		return TRUE;
	else
		return FALSE;
}
...
int Cache::ReplaceDecision(uint64_t index, uint64_t tag) {
	if(debug) printf("Start replace decision %d %ld %ld\n",config_.associativity,index,tag);
...
int Cache::ReplaceAlgorithm(uint64_t index, uint64_t tag){
	if(debug) printf("Begin replace algorithm\n");	
	...
	if(pos==-1){
		int max_time=-1;
		for(int i=0;i<config_.associativity;++i){
			if(come_time[index][i]==-1){
				pos=i;
				break;
			}
			if(come_time[index][i]>max_time){
				pos=i;
				max_time=come_time[index][i];
			}
		}
		come_time[index][pos]=0;
	}
#endif

#ifdef LFU
	if(pos==-1){
		int min_times=10000000;
		for(int i=0;i<config_.associativity;++i){
			if(hit_times[index][i]<min_times){
				pos=i;
				min_times=hit_times[index][i];
			}
		}
		hit_times[index][pos]=0;
	}
#endif

#ifdef RANDOM
	if(pos==-1){
		pos=(rand()%(config_.associativity));
	}
#endif

	if(debug) printf("Replace %d of %ld\n",pos,index);
...
void Cache::ReplaceInfoUpdate(uint64_t index, int pos){
	...
	if(debug) printf("Hit %d of %ld\n",pos,index);
}


void Cache::PrefetchAlgorithm(uint64_t addr){
	if(debug) printf("Start prefetch algorithm\n");

	for(int j=1;j<4;++j){
		uint64_t pre_addr=addr+config_.block_size*j;
		uint64_t offset=pre_addr&((1<<config_.offset_bit)-1);
		uint64_t index=(pre_addr>>config_.offset_bit)&((1<<config_.index_bit)-1);
		uint64_t tag=(pre_addr>>(config_.offset_bit+config_.index_bit))&((1<<config_.tag_bit)-1);
		int pos=ReplaceDecision(index,tag);
		if(pos>=0)
			continue;
		...
		pos=ReplaceAlgorithm(index,tag);
		char *lower_content=new char[config_.block_size];
		//lower_->HandleRequest(pre_addr,1,1,lower_content);
		...
		if(debug) printf("%d: Prefetch %d of %ld, with addr %ld\n",j,pos,index,pre_addr);			
	}
}
